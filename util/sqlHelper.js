const sqlHelper = {
	...
	OracleSelect(table, data = null, cols = [], sort = null) {
		let query = `SELECT * FROM ${table}`;
		const columns = [];
		const where = [];
		const values = [];
		...
		if (data) {
			const keys = Object.keys(data);
			for (key of keys) {
				where.push(`${key}=:` + key.toString()); //where.push(`${key}=?`);
				values.push(data[key]);
			}
			if (where.length > 0 ) {
				query += ` WHERE ` + where.join(' AND ');
		...
		if (cols.length > 0) {			
			//query = query.replace('*', cols.join(', '));
			for (col of cols) {
				columns.push(`${col} "${col.toLowerCase()}"`);
			}
			query = query = query.replace('*', columns.join(', '));
	...
	OracleInsert(table, data) {
		...
		const keys = Object.keys(data);
		//const prepare = new Array(keys.length).fill('?').join(', ');
		//const prepare = Array(keys.length).fill().map((_, index) => ":"+(index )).join(', ');
		const values = [];
		for (const key of keys) {
			values.push(data[key]);
		}
		const prepare = Array(keys.length).fill().map((_, index) => ":"+ keys[index]).join(', ');
		query = query.replace('{1}', keys.join(', '));
		query = query.replace('{2}', prepare);
		return { query, values };
	},
