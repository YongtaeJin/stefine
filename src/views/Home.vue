  ...
  <v-container fill-height fluid> 
    ...
    <v-row v-else class="text-center">
      ...
      <v-col cols="4"/><v-col cols="4"> 
        <v-text-field v-if="stock.M_SU1 < MAX_SU" label="신청가능 주식수" v-model="stock.M_SU2" readonly hide-details />
        <v-text-field v-else label="신청가능 주식수" v-model="MAX_SU" readonly hide-details />
      </v-col><v-col cols="4"/>
      <!-- <v-col cols="4"/><v-col cols="4"> <v-text-field label="양도계약 성립주식수" :model-value="stock.M_SU4 > 0 ? stock.M_SU4 : stock.T_REMARK" readonly  hide-details /></v-col><v-col cols="4"/>  -->
      <v-col cols="4"/><v-col cols="4"> 
        <v-text-field v-if="stock.M_SU4 > 0" label="양도계약 성립주식수" v-model="stock.T_REMARK" readonly  hide-details />
        <v-text-field v-else label="양도계약 성립주식수" v-model="stock.T_REMARK" readonly  hide-details />
      </v-col><v-col cols="4"/> 
...
export default {
  ...
  methods: {
    ...
    async doc(item) {
      ...
      let filenm = ""
      if(item == 'doc1') {
        url = "/upload/2026Y/DOC1.pdf";
        filenm = "자기주식취득에 관한 통지서.pdf"
      } else if (item == 'doc2') {
        url = "/upload/2026Y/DOC2.pdf";
        filenm = "주식양도신청서.pdf"
      } else if (item == 'doc4') {
        url = "/upload/2026Y/DOC4.pdf";
        filenm = "위임장.pdf"      
       
      } else {
        
        // url =  this.stock.f_gun === "대주주" ? "/upload/2025Y/DOC32.pdf" : "/upload/2025Y/DOC31.pdf";
        url =  this.stock.F_GUN === "대주주" ? "/upload/2026Y/DOC31.pdf" : "/upload/2026Y/DOC31.pdf";
